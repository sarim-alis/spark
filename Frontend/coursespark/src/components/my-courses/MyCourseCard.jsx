...
import { Edit, Trash2, Users, DollarSign, BarChart3, Eye } from 'lucide-react';
    ...
    <Card className="flex flex-col h-full border-0 shadow-lg bg-white/80 backdrop-blur-sm">
      ...
      <CardContent className="p-3 md:p-4 flex-grow">
        ...
        <div className="flex items-center gap-2 md:gap-4 text-xs md:text-sm text-slate-500">
          <span className="flex items-center gap-1"><Users className="w-3 h-3 md:w-4 md:h-4" /> {course.total_students || 0}</span>
          <span className="flex items-center gap-1"><DollarSign className="w-3 h-3 md:w-4 md:h-4" /> {(course.total_sales || 0).toFixed(2)}</span>
        </div>
      </CardContent>
