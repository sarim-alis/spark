      ...
      <div className="p-4 md:p-6 lg:p-8 max-w-5xl mx-auto space-y-4 md:space-y-6">
        ...
        <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm">
          <CardHeader className="p-4 md:p-6">
            <CardTitle className="text-xl md:text-xl font-bold">Course Details</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 p-4 md:p-6 pt-0">
            <Input placeholder="Course Title" value={course.title} onChange={(e) => handleInputChange('title', e.target.value)} className="md:col-span-2" />
            <Textarea placeholder="Course Description" value={course.description} onChange={(e) => handleInputChange('description', e.target.value)} className="md:col-span-2"rows={5} />
            <Input type="number" placeholder="Price" value={course.price} onChange={(e) => handleInputChange('price', parseFloat(e.target.value))} />
            ...
            <Input placeholder="Thumbnail URL" value={course.thumbnail_url} onChange={(e) => handleInputChange('thumbnail_url', e.target.value)} className="md:col-span-2" />
            <Input placeholder="External Purchase URL (optional)" value={course.external_url || ''} onChange={(e) => handleInputChange('external_url', e.target.value)} className="md:col-span-2" />
                        ...
                        {(provided) => (
                          <div ref={provided.innerRef} {...provided.draggableProps} className="p-3 md:p-4 border rounded-lg bg-slate-50">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 mb-4">
                              ...
                              <Input value={lesson.title} onChange={(e) => handleLessonChange(index, 'title', e.target.value)} className="font-bold flex-1 text-sm md:text-base" placeholder="Lesson title" />
                              <Input type="number" value={lesson.duration_minutes} onChange={(e) => handleLessonChange(index, 'duration_minutes', parseInt(e.target.value))} className="w-full sm:w-20 md:w-24 text-sm md:text-base" placeholder="Duration" />
                              <Button variant="outline" size="sm" onClick={() => setActiveQuizEditor(activeQuizEditor === index ? null : index)} className={activeQuizEditor === index ? 'bg-amber-100' : ''}>
                                <BrainCircuit className="w-4 h-4 mr-2"/>
                                Quiz
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => removeLesson(index)} className="order-last sm:order-none flex-shrink-0">
                                <Trash2 className="w-4 h-4 text-red-500" />
                              </Button>
                            </div>
                            {activeQuizEditor === index && (
                              <QuizEditor quiz={lesson.quiz} onQuizChange={(updatedQuiz) => handleQuizChange(index, updatedQuiz)} />
                            )}
                            <div className={`[&_.ql-editor]:min-h-[100px] [&_.ql-toolbar]:text-sm ${activeQuizEditor === index ? 'mt-4' : ''}`}>
                              <ReactQuill theme="snow" value={lesson.content} onChange={(content) => handleLessonChange(index, 'content', content)} />
                            </div>
                          </div>
                        )}
                      </Draggable>
        ...
        <div className="flex justify-center md:justify-end">
          <Button onClick={handleSaveCourse} className="w-full md:w-auto bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold shadow-lg text-sm md:text-base">
